...
<body>
	...
	<div id="section">
		<div class="outer">
			...
			<div class="listArea">
				<table id="tableArea">
					<tr>
						<th id="no">글번호</th>
						<th id="title" width="250px">글제목</th>
						<th id="QnaCategory" width="100px">분류</th>
					...
					<% if(list.isEmpty()) { %>
					<tr>
						<th colspan="6" style="text-align:center; font-weight:bold;">존재하는 Q&A가 없습니다.</td>
					</tr>
					<% } else { %>
					<% 		for(Board b : list) { %>
					<tr>
		...
		<div>
			<% if(loginUser != null) {%>		
			<input type="button" onclick="location.href='writeBoardForm.bo'" id="writeNoBtn" value="작성">
			<% } %>
		</div>
	</div>
		...
		$(function() {
			$('#tableArea td').mouseenter(function(){
				...
				$(this).parent().css({'background':'none', 'color':'black'});
	               ...
	               //로그인 한 사람들만 보게하기(게시물) string과 string으로 비교하여 만들어야함 'null'
	               if('<%= loginUser %>' !='null'){
